...
import { TIER_SCORES, type BalancedTeams, type HealthStatus, type InhouseMatch, type InhouseTier, type MatchParticipant, type Player, type PlayerInput, type Position } from './types';
...
function PlayersPage({ data, loading, error, reload }: ReturnType<typeof useAppData>) {
  ...
  const [editing, setEditing] = useState<Player | null | undefined>(); const [form, setForm] = useState<PlayerInput>(EMPTY_PLAYER); const [busy, setBusy] = useState(''); const [message, setMessage] = useState('');
  ...
  async function refreshOne(id: string) { if (!getRiotApiKey() && !data.health?.riotConfigured) { setMessage(t('keyMissing')); return; } setBusy(id); try { const result = await api.refreshPlayer(id); setMessage(result.warnings.join(' ')); await reload(); } catch (cause) { setMessage(cause instanceof Error ? cause.message : String(cause)); } finally { setBusy(''); } }
