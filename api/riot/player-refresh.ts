...
export default handler(async (req, res) => {
  ...
  const account = await getRiotAccount(String(existing.riot_id), key);
  const solo = await getSoloRank(account.puuid, key);
  const now = new Date().toISOString();
  const patch: Record<string, unknown> = {
    puuid: account.puuid, riot_game_name: account.gameName, riot_tag_line: account.tagLine, riot_id: `${account.gameName}#${account.tagLine}`,
    ...
    riot_last_updated_at: now, updated_at: now,
  };
  const warnings: string[] = [];
  try {
    const history = await getOpggHistoricalRanks(`${account.gameName}#${account.tagLine}`);
    Object.assign(patch, {
